...
### `ci.yml` — "CI"
- **Build & Test** (`ubuntu-24.04`): `vcpkg install` → `cmake --preset ci` → build → `ctest --preset ci`.
  (The `ci` preset is Release, `BUILD_TESTS=ON`, `BUILD_EXAMPLES=OFF`.)
...
4. **Pin/trust actions deliberately.** Some are pinned (`actions/checkout@v4`,
   `codeql-action/upload-sarif@v3`); `trivy-action@master` is intentionally floating — prefer
   pinning when you touch it.
