...
///   });
///   server.listen("127.0.0.1", 8080);  // bind all interfaces only if intended
/// @endcode
class RestServer {
  public:
    ...
    struct Request {
        ...
        [[nodiscard]] std::string query(const std::string &key, const std::string &def = "") const {
    ...
    using Middleware = std::function<void(const httplib::Request &, httplib::Response &)>;
    ...
    void route(const char *method, const std::string &pattern, Handler h) {
        auto wrapped = [this, h = std::move(h)](const httplib::Request &req,
            ...
            for (auto &mw : middleware_) {
                mw(req, res);
                // Non-200 status short-circuits: handler is not invoked.
                // (httplib defaults status to 200; CORS/logger do not change it.)
                if (res.status != 200)
                    return;
            }
            Request wreq{req};
            ...
            try {
                ...
                wres.set_json(Status::BadRequest, {{"error", "invalid JSON"}});
#endif
            } catch (const std::exception &) {
                // Do not leak exception messages to clients.
#ifdef HAS_JSON
                wres.set_json(Status::InternalError, {{"error", "internal error"}});
#else
                wres.set_text(Status::InternalError, "internal error");
#endif
            }
        };

        if (std::string(method) == "GET")
            server_.Get(pattern, wrapped);
        else if (std::string(method) == "POST")
            server_.Post(pattern, wrapped);
        else if (std::string(method) == "PUT")
            server_.Put(pattern, wrapped);
        else if (std::string(method) == "DELETE")
            server_.Delete(pattern, wrapped);
    }
