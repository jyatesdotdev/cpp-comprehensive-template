...
template <typename Range> struct Source {
...
template <typename Prev, typename Fn> struct Map {
...
template <typename Prev, typename Pred> struct Filter {
    Prev prev; ///< Previous stage.
...
template <typename Prev, typename Fn> struct FlatMap {
    Prev prev; ///< Previous stage.
...
/// @tparam Prev Previous pipeline stage type.
template <typename Prev> struct Take {
    Prev prev;     ///< Previous stage.
...
template <typename R> struct element_type<Source<R>> {
    using type = typename R::value_type;
};

template <typename P, typename Fn> struct element_type<Map<P, Fn>> {
    using type = std::invoke_result_t<Fn, typename element_type<P>::type>;
};

template <typename P, typename Pred> struct element_type<Filter<P, Pred>> {
    using type = typename element_type<P>::type;
};

template <typename P, typename Fn> struct element_type<FlatMap<P, Fn>> {
    using type = typename std::invoke_result_t<Fn, typename element_type<P>::type>::value_type;
};

template <typename P> struct element_type<Take<P>> {
    using type = typename element_type<P>::type;
};

template <typename S> using element_type_t = typename element_type<S>::type;
...
template <typename Stage> class Pipeline {
  public:
    ...
    template <typename Pred> auto filter(Pred pred) const {
    ...
    template <typename Fn> auto flat_map(Fn fn) const {
    ...
    auto take(std::size_t n) const { return Pipeline<Take<Stage>>{Take<Stage>{stage_, n}}; }
    ...
    template <typename Out = void> auto collect() const {
    ...
    template <typename T, typename BinOp> T reduce(T init, BinOp op) const {
        evaluate(stage_,
                 [&](auto &&v) { init = op(std::move(init), std::forward<decltype(v)>(v)); });
        return init;
    }
    ...
    template <typename Fn> void for_each(Fn fn) const { evaluate(stage_, fn); }
  ...
  private:
    ...
    template <typename R, typename Cb> static void evaluate(const Source<R> &s, Cb &&cb) {
        for (auto &&e : s.data)
            cb(e);
    }
    ...
    static void evaluate(const FlatMap<P, Fn> &s, Cb &&cb) {
        evaluate(s.prev, [&](auto &&v) {
            for (auto &&inner : s.fn(std::forward<decltype(v)>(v)))
                cb(inner);
        });
    }

    template <typename P, typename Cb> static void evaluate(const Take<P> &s, Cb &&cb) {
...
template <typename Range> auto from(const Range &data) {
