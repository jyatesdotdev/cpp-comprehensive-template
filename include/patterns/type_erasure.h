...
///
/// Heap-allocated value type that can hold any callable/drawable/etc.
...
class Drawable {
  ...
  public:
    ...
    [[nodiscard]] std::string draw() const {
        if (!impl_)
            throw std::runtime_error("empty Drawable");
        return impl_->draw();
    }
};
...
template <typename R, typename... Args> class Function<R(Args...)> {
  ...
  public:
    ...
    template <typename F> Function(F f) : impl_(std::make_unique<Model<F>>(std::move(f))) {}
