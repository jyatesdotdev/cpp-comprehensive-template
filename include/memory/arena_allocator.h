...
class Arena {
  public:
    ...
    explicit Arena(std::size_t capacity) : capacity_{0}, buf_{nullptr} {
        if (capacity == 0)
            return;
...
template <typename T> class ArenaAllocator {
  public:
    ...
    template <typename U> ArenaAllocator(const ArenaAllocator<U> &o) noexcept : arena_{o.arena_} {}
    ...
    template <typename U> bool operator==(const ArenaAllocator<U> &o) const noexcept {
