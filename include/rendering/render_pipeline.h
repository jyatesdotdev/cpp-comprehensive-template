...
class RenderPass {
  public:
    ...
    RenderPass(GLuint fbo, int width, int height,
        ...
        glGetIntegerv(GL_VIEWPORT, prev_viewport_.data());
    ...
    ~RenderPass() {
        ...
        glViewport(prev_viewport_[0], prev_viewport_[1], static_cast<GLsizei>(prev_viewport_[2]),
                   static_cast<GLsizei>(prev_viewport_[3]));
    }
  ...
  private:
    GLint prev_fbo_ = 0;
    std::array<GLint, 4> prev_viewport_{};
};
